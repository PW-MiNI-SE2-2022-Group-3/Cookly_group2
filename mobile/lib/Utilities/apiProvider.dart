...
class CooklyProvider extends GetConnect {
  ...
  var AuthToken = '';
  var ingredientUrl2 =
      Uri.parse('http://localhost:3001/ingredients?page=0&limit=5000');
  ...
  Future<List<Recipe>> fetchRecipes(
      List<Ingredient> args, http.Client client) async {
  ...
  Future loginMethod(
    ...
    var body = jsonEncode(
        {"username": username, "password": passwordCrypted.toString()});
  ...
  Future registerMethod(
    ...
    var body = jsonEncode(
        {"username": username, "password": passwordCrypted.toString()});
    var res = await client.post(Uri.parse('http://localhost:3001/registration'),
        headers: {"Content-Type": "application/json"}, body: body);
    print('res : ${res.statusCode}');
  ...
  Future<Response> getIngredients() =>
      get('http://localhost:3001/ingredients/next?page=0&limit=5000');
  Future<Response> postngredients() => post(
